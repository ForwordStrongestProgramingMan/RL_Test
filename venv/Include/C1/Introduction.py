class Environment():
    ...
    def obs_space(self):
        """根据自身状态空间以及交互的个体身份确定该个体所具有的观测空间"""
        # 可默认具有环境的状态空间，此时对个体老说是完全可观测环境
        print("环境：开放观测空间给个体")
...
class Agent():
    """个体类"""
    def __init__(self, env=None, name="agent_1"):
        self.env = env
        self.name = name
    ...
    def model(self, action=None):
        print("个体：思考行为可能带来的下一个时刻的观测、及时奖励以及是否交互结束")
    ...
    def act(self, action_0):
        # 调用环境的动力学方法
        print("个体：执行一个行为")
        obs_1, reward_1, is_done = self.env.dynamics(action_0)  # 环境根据 action_0 产生obs_1, reward_1
