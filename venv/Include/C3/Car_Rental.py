...
# 动态规划算法时间复杂度随着状态数量和行为数量的增多而处平方级别增多
# 本例中的状态数为21*21=441,行为数为11,按照动态规划算法完成一次迭代花费的计算资源较为昂贵
...
def dynamic(s, a):
    ...
    Args:
        ...
        a 转场汽车数 int 定义为从 loc1转运a辆车至 loc2，负数表示反向搬运 -5<=a<=5
    Return:
        s_prime 后续状态 tuple(int, int) (n1, n2)
        reward float 奖励，两地合算，每租出1辆车，奖励为10
...
def _need_return(car_rent, loc_s0, loc_s1):
    ...
    """
    return car_rent + (loc_s1 - loc_s0)  # loc_s1 - (loc_s0 - car_rent)
...
def prob_one_location(n_rent, n_return, loc_s0, loc_s1):
    ...
    prob = 0  # 初始概率设为0
    for car_rent in range(0, max_for_rent):  # 对于每一个借出去的可能性
        prob_rent = poisson_prob(n_rent, car_rent)
        # 借出car_rent辆车，需要有car_return辆返回才能满足最终汽车数量为loc_s1
        car_need_return = _need_return(car_rent, loc_s0, loc_s1)
        if car_need_return < 0:  # 说明借的数量不够多，将在借出里考虑计算，不重复计算
            pro_return = 0.0
            # continue
        pro_return = poisson_prob(n_return, car_need_return)
        prob += prob_rent * pro_return
    return prob
...
def P(s, a, s1):
    ...
    if not (0 <= n1 <= MAX_CARS and 0 <= n2 <= MAX_CARS):
        # 初始状态不合法
        return 0.0
    n1_aftmv, n2_aftmv = n1 - a, n2 + a  # 移动a辆汽车后，两地的汽车数量
...
def compute_q(MDP, V, s, a):
    ...
    q_sa = 0
    for s_prime in S:
        q_sa += get_prob(P, s, a, s_prime) * get_value(V, s)
...
V_pi = policy_evaluate(MDP, V, uniform_random_pi, 20)
display_V(V_pi)
