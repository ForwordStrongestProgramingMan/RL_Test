...
def dynamics(s, a):  # 环境动力学
    ...
    :param a: 行为 str in ['n','e','s','w'] 分别表示北、东、南、西
    ...
    '''
    s_prime = s
    # 判断该状态是否在最边缘地带，并且方向往边缘之外
    if (s % 4 == 0 and a == 'w') or (s < 4 and a == 'n') \
            or ((s + 1) % 4 == 0 and a == 'e') \
            or (s > 11 and a == 's') or s in [0, 15]:
        pass
    else:
        ds = ds_actions[a]
        s_prime = ds + s  # 获得后续状态
...
def greedy_pi(MDP, V, s, a):
    ...
    for a_opt in A:  # 统计后续状态的最大价值以及到达该状态的行为(可能不止一个)
        ...
        v_s_prime = get_value(V, s_prime)  # 记录后续状态的价值
        if v_s_prime > max_v:  # 保存后续状态中最大价值的后续状态
            max_v = v_s_prime
            a_max_v = [a_opt]
        elif v_s_prime == max_v:  # 存在多个最大价值的后续状态
            a_max_v.append(a_opt)
    n = len(a_max_v)
    if n == 0:
        return 0.0
    return 1.0 / n if a in a_max_v else 0.0  # 多个最大价值进行均匀随机策略
...
def compute_q(MDP, V, s, a):
    '''根据给定的MDP，价值函数，计算状态行为对s,a的价值qsa
    '''
    S, A, R, P, gamma = MDP
    q_sa = 0
    for s_prime in S:
...
def compute_v(MDP, V, Pi, s):
    '''根据MDP下依据某一策略Pi和当前状态价值函数V计算某状态s的价值
    '''
    S, A, R, P, gamma = MDP
    v_s = 0
    for a in A:
...
def policy_evaluate(MDP, V, Pi, n):
    '''使用n次迭代计算来评估一个MDP在给定策略Pi下的状态价值，初始价值为V
    '''
    for i in range(n):
...
V = [0 for _ in range(16)]  # 初始化状态价值为零
display_V(V)
...
def compute_v_from_max_q(MDP, V, s):
    '''根据一个状态下的所有可能的行为价值中最大一个来确定当前状态值
    '''
    S, A, R, P, gamma = MDP
    v_s = -float('inf')
    for a in A:
...
def update_V_without_pi(MDP, V):
    ...
    V_prime = V.copy()
    for s in S:
        set_value(V_prime, s, compute_v_from_max_q(MDP, V, s))
    return V_prime
